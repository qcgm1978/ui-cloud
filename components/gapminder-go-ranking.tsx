...
const GapminderGoRankingComponent = React.memo(() => {
      ...
      const processedData = yearsRef.current.reduce((acc, year) => {
        const yearData = data
          ...
          .map((row, index) => ({
            ...
            rating:
              row[headers.indexOf(year)] !== null
                ? Number(row[headers.indexOf(year)])
                : null,
          }))
  ...
  const updateChart = useCallback(
    (year: string) => {
      ...
      const r = scaleLinear().range([5, 40]).domain([minRating, maxRating]);
      ...
      const simulation = forceSimulation(yearData)
        .force(
          ...
          forceX((d: GoPlayer) =>
            Math.max(
              margin.left + r(d.rating as number),
              Math.min(
                width - margin.right - r(d.rating as number),
                x(year) || 0
              )
            )
          ).strength(0.1)
        ...
        .force(
          ...
          forceY((d: GoPlayer) =>
            Math.max(
              margin.top + r(d.rating as number),
              Math.min(
                height - margin.bottom - r(d.rating as number),
                y(d.rating as number)
              )
            )
          ).strength(0.1)
        )
        .force(
          "collide",
          forceCollide((d: GoPlayer) => r(d.rating as number) + 1).iterations(4)
        )
        .stop();
      ...
      // 更新玩家
      const players = svg
        .selectAll<SVGGElement, GoPlayer>("g.player")
      ...
      // 添加新的玩家
      const enterPlayers = players
        .enter()
        .append("g")
        .attr("class", "player")
        .attr(
          "transform",
          (d: GoPlayer) => `translate(${(d as any).x},${(d as any).y})`
        )
        .style("opacity", 0);
  ...
  return (
    <div className="w-screen h-screen overflow-hidden">
      <div className="text-red-500">
        GapminderGoRankingComponent is rendering
      </div>
